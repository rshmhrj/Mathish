...
public class Problem9
	{
		 ...
		 *            b c, for which, a2 + b2 = c2 For example, 32 + 42 = 9 + 16
		 *            = 25 = 52. There exists exactly one Pythagorean triplet
		...
		public static void body( long x )
			...
			for ( int a = 1; a < 10 * x; a++ )
				...
				for ( int b = 1; b < 10 * x; b++ )
					...
					if ( Maths.isSqrtInteger( ( Math.pow( a, 2 ) + Math.pow(	b,
																				2 ) ) ) )
						{
						if ( !Maths.isPythagoreanMultiple( triples, a, b, c )
								&& a + b + c == (int) x )
							{
							triples[found][0] = a;
							triples[found][1] = b;
