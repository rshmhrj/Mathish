...
//		{
//			ans += primes.get(i);
////			if (i==0) additions += additions+(Integer.toString(primes.get(i)));
////			else if (i==primes.size()-1) additions += (" + "+Integer.toString(primes.get(i)));
	...
	private static double countPrimesFast(int count) throws Exception
	{
		...
		for(int i = 10; i < count; i += 10)
		{
			...
			for(int j : step)
			{
				...
				{
					if(prevPrime > sqrt){break;}
					if(k % prevPrime == 0){continue step;}
				}
				sum+= k;
