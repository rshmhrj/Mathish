...
//		int t9 = 1283379;
		...
		System.out.println("Time for population & sorting = " + Long.toString(myEndTime - myStartTime));
				...
				for (int k = 0; k < n; k++){
					if (j != k){
						if (targetSums[i] - A[k] == A[j]){
							result[i] = true;
							System.out.println("Found sum for "+Integer.toString(targetSums[i])+" = "+A[j]+" + "+A[k]);
							break;
						}
					}
				}
				if (result[i] == true){
//					System.out.println("Finished with" + Integer.toString(targetSums[i]));
					break;
				}
			}
			long myLoopEndTime = System.currentTimeMillis();
//			System.out.println("Time for loop["+i+"] = " + Long.toString(myLoopEndTime - myLoopStartTime));

		}
		long loopEndTime = System.currentTimeMillis();
		
		System.out.println("Result = " + toString(result));
		System.out.println("Total time taken = " + Long.toString(loopEndTime - loopStartTime));
