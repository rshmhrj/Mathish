...
public class countInversions
{
	 ...
	 * numerical order in the list. We use a modified version of the MergeSort
	 * algorithm to do this, so it's a recursive function. We split the list
	 * into two (almost) equal parts, recursively count the number of inversions
	 * in each part, and then count inversions caused by one element from each
	 * part of the list. The merging is done is a separate procedure given
	 * below, in order to simplify the presentation of the algorithm here. Note:
	 * I am assuming that the integers are distinct, but they need *not* be
