...
public class countInversions
	{
		...
		public static void body() throws IOException
			...
			System.out.println( "Number of Inversions (my code)    = "
								+ invCount );
			...
			switch ( A_length )
				{
					...
					default:
						...
						A = mergeAndCountSplitInv(	A_length, B, B_length, C,
													C_length );
						return A;
						}
				}
		...
		/*
		 * This function will count the number of inversions in an array of
		 * numbers. (Recall that an inversion is a pair of numbers that appear
		 * out of numerical order in the list. We use a modified version of the
		 * MergeSort algorithm to do this, so it's a recursive function. We
		 * split the list into two (almost) equal parts, recursively count the
		 * number of inversions in each part, and then count inversions caused
		 * by one element from each part of the list. The merging is done is a
		 * separate procedure given below, in order to simplify the presentation
		 * of the algorithm here. Note: I am assuming that the integers are
		 * distinct, but they need *not* be integers { 1, 2, ..., n} for some n.
		 */
			{
			...
			/*
			 * Otherwise, we create new arrays and split the list into two
			 * (almost) equal parts.
			 */
			...
			/*
			 * Now merge the two sublists together, and count the inversions
			 * caused by pairs of elements, one from each half of the original
			 * list.
			 */
			...
			/*
			 * Finally, put the resulting list back into the original one. This
			 * is necessary for the recursive calls to work correctly.
			 */
		...
		public static long
				mergeAndCount( int left[], int right[], int result[] )
		/*
		 * This procudure will merge the two lists, and count the number of
		 * inversions caused by the elements in the "right" list that are less
		 * than elements in the "left" list.
		 */
			{
			int a = 0, b = 0, i, k = 0;
			long count = 0;
