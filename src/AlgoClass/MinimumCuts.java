...
public class MinimumCuts
	{
		...
		public static int[][]
				contract( int[][] graph, int[] edge, int iteration )
			{
			int contractedVertex = 1000 * iteration;
			// Integer.parseInt((Integer.toString(edge[0]) +
			...
			for ( int[] vertex : graph )
				...
				if ( vertex[0] != edge[0] && vertex[0] != edge[1] )
					...
					for ( int insideVertex = 1; insideVertex < vertex.length; insideVertex++ )
						{
						if ( vertex[insideVertex] == edge[0]
								|| vertex[insideVertex] == edge[1] )
							{
							vertex[insideVertex] = contractedVertex;
							}
						}
					result[i] = new int[ modifiedVertexLength ];
					System.arraycopy(	vertex, 0, result[i], 0,
										modifiedVertexLength );
					i++;
					}
				else
					{
		...
		public static long[][] contract( long[][] graph, int[] edge,
											int iteration )
			{
